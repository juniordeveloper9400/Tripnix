...
function syncAccEntryKind(kind) {
  ...
  setDropdown('acc-entry-category-dd', {
    groups: cats?.groups?.[kind],
    flat: cats?.categories?.[kind],
    // Preserved across a kind change when the same category exists in both,
    // rather than silently resetting a choice the user already made.
    value: document.getElementById('acc-entry-category')?.value,
  ...
  setDropdown('acc-entry-vehicle-dd', {
    ...
    value: document.getElementById('acc-entry-vehicle')?.value,
...
function renderTracking() {
  ...
  document.getElementById('gps-list').innerHTML = vehicles.length
    ? vehicles.map(v => {
        ...
        const driver = detail ? `<div class="diary-row-who">${detail}</div>` : '';
        ...
        return `
          <div class="diary-row gps-row">
            <div class="diary-row-main">
              ...
              <code class="vehicle-number">${escapeHtml(v.subtitle || '—')}</code>
              <div class="diary-row-who">${where}${link}</div>
              ${driver}
              <div class="diary-row-status">${badge}</div>
            </div>
            <div class="row-map" data-bus-map="${v.id}"></div>
          </div>`;
      }).join('')
  ...
  for (const v of vehicles) {
    renderBusMiniMap(
      document.querySelector(`[data-bus-map="${v.id}"]`),
      v,
      { sample }
    );
  }
